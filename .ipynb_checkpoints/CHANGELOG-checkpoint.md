...
An example notebook is provided in the [Language Models](https://github.com/yui-mhcp/language_models) project to build and run workflows !

Workflows, or "agentic LLM", is a way to build complex execution graphs by using LLM to take some decisions to take one or another path in the execution. It can be seen like a new programmation framework in which you can take complex decisions like asking the model "should you go on the internet ?", then "write a query to search on the web", etc. The provided example is a simple `pdf tagger`, but the next update will add new examples like web search, document analysis, etc. Please open a discussion in [the repo](https://github.com/yui-mhcp/language_models) to propose ideas ! :yum:
